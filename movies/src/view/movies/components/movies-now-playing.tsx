...
export function MoviesNowPlaying() {
  const { data: moviesNowPlaying, isLoading } = useQuery<TmdbProps[]>({
  ...
  <div className="flex justify-between items-center px-2 py-4">
    <h3 className="text-primary text-lg lg:text-xl">Filmes novos</h3>  
    </div> 
    <Carousel>
      <CarouselContent className="-ml-2 py-2 md:-ml-4">
        {moviesNowPlaying &&
          moviesNowPlaying.map((movie: TmdbProps) =>
           (
            <Link 
             to={`/movie/${movie.id}`}
             key={movie.id}
