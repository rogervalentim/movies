...
export function MoviesPopular() {
  const { data: moviesPopular, isLoading } = useQuery<TmdbProps[]>({
  ...
  return (
    ...
    <div className="flex justify-between items-center px-2 py-4">
      <h3 className="text-primary text-lg lg:text-xl">Filmes populares</h3>  
      </div>   
    <Carousel>
      <CarouselContent className="-ml-2 py-2 md:-ml-4">
        {moviesPopular &&
          moviesPopular.map((movie: TmdbProps) =>
           (
